<div class="h-[calc(100vh-140px)] flex flex-col md:flex-row bg-white rounded-3xl border border-gray-100 shadow-sm overflow-hidden" wire:poll.3s>
    ...
    <div class="w-full md:w-80 border-r border-gray-100 flex flex-col h-full bg-[#FAF8F2]/60">
        <div class="p-4 border-b border-gray-100">
            <h3 class="font-display font-bold text-sm text-[#0B5A45]">Chat Pelanggan Toko</h3>
        </div>
            ...
            @forelse($chats as $c)
                <button type="button" wire:click="selectChat({{ $c->id }})" class="w-full p-3.5 text-left flex items-start gap-3 transition {{ $selectedChatId === $c->id ? 'bg-[#EDFDF5] border-l-4 border-[#0E9F6E]' : 'hover:bg-white/80' }}">
                    <div class="w-10 h-10 rounded-2xl bg-[#0E9F6E] text-white font-bold flex items-center justify-center text-xs shrink-0 shadow-xs">
                        {{ substr($c->buyer->name ?? 'P', 0, 1) }}
                    </div>
                    <div class="min-w-0 flex-1">
                        <div class="flex items-center justify-between">
                            <h4 class="font-bold text-xs text-[#1E2723] truncate">{{ $c->buyer->name }}</h4>
                            <span class="text-[9px] text-gray-400 font-mono">{{ $c->last_message_at?->format('H:i') }}</span>
                        </div>
                        <p class="text-[11px] text-gray-400 truncate mt-0.5">
                            {{ $c->messages->first()?->body ?? 'Mulai percakapan...' }}
                        </p>
                    </div>
                </button>
            @empty
                <div class="p-8 text-center text-xs text-gray-400">Belum ada chat masuk dari pelanggan.</div>
            @endforelse
        </div>
    </div>
    ...
    <div class="flex-1 flex flex-col h-full bg-white">
        @if($selectedChat)
            ...
            <div class="p-4 border-b border-gray-100 flex items-center justify-between bg-white">
                <div class="flex items-center gap-3">
                    <div class="w-10 h-10 rounded-2xl bg-[#0E9F6E] text-white font-bold flex items-center justify-center text-sm">
                        {{ substr($selectedChat->buyer->name, 0, 1) }}
                    </div>
                    <div>
                        <h4 class="font-bold text-xs text-gray-900">{{ $selectedChat->buyer->name }}</h4>
                ...
                @if($selectedChat->product)
                    <div class="text-right hidden sm:block">
                        <span class="text-[10px] text-gray-400 block">Menanyakan:</span>
                        <span class="text-xs font-bold text-[#0B5A45]">{{ $selectedChat->product->name }}</span>
                    </div>
                @endif
            </div>
            ...
            <form wire:submit="sendMessage" class="p-3 border-t border-gray-100 bg-white flex gap-2">
                <input type="text" wire:model="messageText" placeholder="Ketik balasan untuk pelanggan..." class="flex-1 px-4 py-2.5 bg-[#FAF8F2] border border-gray-200 rounded-2xl text-xs focus:border-[#0E9F6E] focus:outline-none">
        ...
        @else
            <div class="flex-1 flex flex-col items-center justify-center text-gray-400 p-8 text-center">
                ...
                <p class="text-xs">Pilih percakapan untuk membalas pesan.</p>
            </div>
        @endif
    </div>
